...
export function SavingsForm({ onSuccess }: SavingsFormProps) {
  ...
  const onSubmit = async (data: SavingsSchema) => {
    setIsLoading(true);
    const result = await addSavings(data, settings.google_sheet_id, settings.sheet_tabs.tabungan);
    setIsLoading(false);
    if (result.success) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} noValidate style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      <div className="form-group">
        ...
        <input type="text" placeholder="Contoh: Liburan ke Bali" className={`input ${errors.nama_tujuan ? 'input-error' : ''}`} {...register('nama_tujuan')} />
        {errors.nama_tujuan && <span className="form-error">{errors.nama_tujuan.message}</span>}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
        <div className="form-group">
          ...
          <input type="date" className={`input ${errors.target_tanggal ? 'input-error' : ''}`} {...register('target_tanggal')} />
          {errors.target_tanggal && <span className="form-error">{errors.target_tanggal.message}</span>}
        </div>
          ...
          {SAVINGS_ICONS.map((icon) => (
            <button
              ...
              style={{
                ...
                background: selectedIcon === icon ? 'var(--color-surface-offset)' : 'transparent',
                border: selectedIcon === icon ? '2px solid var(--color-primary)' : '1px solid var(--color-border)',
          ...
          {SAVINGS_COLORS.map((color) => (
            <button
              ...
              style={{
                ...
                boxShadow: selectedColor === color ? `0 0 0 2px var(--color-surface), 0 0 0 4px ${color}` : 'none',
                transition: 'all var(--transition)'
              }}
            />
          ))}
      ...
      <div className="form-group">
        ...
        <input type="text" placeholder="Opsional..." className="input" {...register('deskripsi')} />
      </div>
