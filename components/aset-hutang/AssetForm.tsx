...
export function AssetForm({ onSuccess, initialData }: AssetFormProps) {
  ...
  const onSubmit = async (data: AssetSchema) => {
    setIsLoading(true);
    const result = initialData ? await updateAsset(initialData.id, data) : await addAsset(data);
    setIsLoading(false);
    if (result.success) {
      toast.success(initialData ? "Aset diperbarui!" : "Aset berhasil dicatat!");
      onSuccess?.();
    } else {
      toast.error(result.error || (initialData ? "Gagal memperbarui aset" : "Gagal mencatat aset"));
    }
  };
    ...
    <form onSubmit={handleSubmit(onSubmit)} noValidate style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
      <div className="form-group">
        ...
        <input
          ...
          {...register("nama")}
        />
        {errors.nama && <span className="form-error">{errors.nama.message}</span>}
      </div>

      <AssetTypeSelector selectedJenis={selectedJenis} register={register} />

      {selectedJenis === "emas" && (
        <div style={{ padding: "0.875rem", background: "#fff9db", borderRadius: "var(--radius-lg)", border: "1px solid #fab005", display: "flex", flexDirection: "column", gap: "0.25rem" }}>
          <div style={{ fontSize: "0.8125rem", fontWeight: 700, color: "#856404", display: "flex", alignItems: "center", gap: "0.375rem" }}>
            <span>💡</span> Tips Pencatatan Emas
          </div>
          <p style={{ fontSize: "0.75rem", color: "#856404", margin: 0 }}>
            Anda bisa mencatat <strong>berat (gram)</strong> dan <strong>harga beli</strong> di kolom Catatan di bawah agar riwayatnya lebih lengkap.
          </p>
        </div>
      )}
      ...
      <div className="form-group">
        <Controller
          ...
          render={({ field: { onChange, value } }) => (
            <CurrencyInput label="Nilai Aset Saat Ini" required value={value} onChange={onChange} error={errors.nilai?.message} />
          )}
        />
      </div>
      ...
      <div className="form-group">
        ...
        <input type="date" className={`input ${errors.tanggal_update ? "input-error" : ""}`} {...register("tanggal_update")} />
        {errors.tanggal_update && <span className="form-error">{errors.tanggal_update.message}</span>}
      </div>
